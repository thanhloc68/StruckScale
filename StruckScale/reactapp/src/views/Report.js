...
const Report = () => {
    ...
    const getListType = async (selectValue, newStartDate, newEndDate) => {
        ...
        //const res = await axios(url + ':7007/api/Report/getlist/' + selectValue).then(x => { setStruckScale(x.data) }).catch(e => { console.log(e), setStruckScale([]) })
        newStartDate = startDate ? startDate.toLocaleDateString("en-CA") : null;
        newEndDate = endDate ? endDate.toLocaleDateString("en-CA") : null;
        const res = await axios(url + ':7007/api/Report/getlist/' + selectValue + '?startDate=' + newStartDate + '&endDate=' + newEndDate + '&search=' + searchInput).then(x => { setStruckScale(x?.data ?? []), setdataExcel(x?.data ?? []) }).catch(e => { console.log(e), setStruckScale([]) })
    };
    ...
    return (
        <>
            ...
            <div className="tablebox">
                <div className="typeReport">
                    <span>Chọn loại Report: </span>
                    <select value={getDataInput.name} onChange={(e) => handleChangeOn(e)} >
                        <option value=" ">...</option>
